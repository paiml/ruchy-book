#!/usr/bin/env bash
# Run BENCH-009 (JSON Parsing) in all 10 execution modes
...
readonly BENCH_NAME="JSON parsing (50MB file)"
readonly PYTHON_SCRIPT="$(dirname "$0")/bench-009-json-parsing.py"
readonly DENO_SCRIPT="$(dirname "$0")/bench-009-json-parsing.ts"
readonly JULIA_SCRIPT="$(dirname "$0")/bench-009-json-parsing.jl"
readonly GO_SCRIPT="$(dirname "$0")/bench-009-json-parsing.go"
readonly RUST_SCRIPT="$(dirname "$0")/bench-009-json-parsing-rust.rs"
readonly C_SCRIPT="$(dirname "$0")/bench-009-json-parsing.c"
readonly RUCHY_SCRIPT="$(dirname "$0")/bench-009-json-parsing.ruchy"
readonly RESULTS_FILE="$(dirname "$0")/results/bench-009-results-full.json"

echo "========================================" >&2
echo "BENCH-009: JSON Parsing Benchmark (Full)" >&2
...
# Build JSON output
echo "{" > "$RESULTS_FILE"
echo '  "benchmark": "BENCH-009",' >> "$RESULTS_FILE"
echo '  "name": "JSON parsing (50MB file)",' >> "$RESULTS_FILE"
...
# Mode 6: C (compiled) - Skip if no C implementation
if [ -f "$C_SCRIPT" ]; then
    echo '    "c": ' >> "$RESULTS_FILE"
...
echo "" >&2
echo "✅ BENCH-009 complete! Results saved to:" >&2
echo "   $RESULTS_FILE" >&2
...
with open('results/bench-009-results-full.json') as f:
    data = json.load(f)

print("\n" + "="*70)
print(f"BENCH-009 RESULTS: {data['name']}")
print("="*70)
