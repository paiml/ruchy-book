#!/usr/bin/env bash
# Run BENCH-012 (Startup Time) in all 10 execution modes
...
readonly BENCH_NAME="Startup time (Hello World)"
readonly PYTHON_SCRIPT="$(dirname "$0")/bench-012-startup.py"
readonly DENO_SCRIPT="$(dirname "$0")/bench-012-startup.ts"
...
# Build JSON output
echo "{" > "$RESULTS_FILE"
echo '  "benchmark": "BENCH-012",' >> "$RESULTS_FILE"
echo '  "name": "Startup time (Hello World)",' >> "$RESULTS_FILE"
...
with open('results/bench-012-results-full.json') as f:
    data = json.load(f)
...
print("\n📖 Note: Startup time is critical for CLI tools and short-running scripts")
print("   Compiled binaries (Go, Rust, Ruchy Compiled) should be fastest")
print("   Interpreters (Python, AST) have runtime initialization overhead")
EOF
