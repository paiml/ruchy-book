# 🚨 CRITICAL: Variable Scoping Bug WORSENED in Trunk (v3.213.0+)

**Date**: 2025-09-24
**Trunk Commit**: 7380d538 [P0-FIX] Fix #10: Preserve test attributes during compilation
**Version**: 3.45.0 (trunk)
**Severity**: 🔴🔴 CRITICAL - Even worse than released v3.213.0

## Executive Summary

The variable scoping bug in v3.213.0 has **gotten WORSE** in trunk. Now even function-wrapped code has incorrect scoping due to excessive block nesting. The transpiler is generating deeply nested blocks that trap variables in inaccessible scopes.

## Comparison: Released v3.213.0 vs Trunk
...
- Released v3.213.0: Function-wrapped code worked, only top-level failed
