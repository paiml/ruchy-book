...
## Bug #002: Main Function Incorrect Return Type in v1.29.1
...
### Description
The Ruchy compiler v1.29.1 incorrectly generates `fn main() -> i32` instead of `fn main()` in the transpiled Rust code, causing all basic programs to fail compilation with type mismatch errors.
...
- Regression from v1.29.1 which worked correctly

### Workaround
None available. Must revert to v1.29.1 or wait for fix.

### Related Commit
- 901910b [BUG-002] v1.29.1 - Fix higher-order function transpilation
...
### Resolution
Fixed in ruchy v1.29.1! The file operations now work correctly:
```bash
$ ruchy --version
ruchy 0.7.10
...
### Description
While the v1.29.1 parser can successfully parse function definitions using the `fun` keyword, the interpreter fails to execute files containing function definitions. The parser generates correct AST but execution fails with "Failed to parse input" error.
...
### Workaround
None currently available. Functions cannot be executed in v1.29.1.
...
**Resolution**: Fixed in ruchy v1.29.1! Array indexing now works correctly:
```bash
$ ruchy --version
ruchy 0.9.0
...
## Bug #004: CRITICAL - v1.29.1 Release Compilation Failure
...
### Description
The v1.29.1 release cannot compile due to incomplete macro system implementation. The `Macro` variant was added to `ExprKind` enum and the type inference match arm calls `self.infer_macro(name, args)`, but the `infer_macro` method is not implemented in the `InferenceContext` struct.
...
### Impact Assessment
- **BREAKING**: v1.29.1 cannot be installed or used
...
1. **URGENT**: Revert v1.29.1 from crates.io if possible
...
**Resolution**: Fixed in ruchy v1.29.1! The missing `infer_macro` method was implemented:
```bash
$ ruchy --version
ruchy 0.9.8
...
**Impact of Fix**:
- v1.29.1 compiles and runs correctly
...
|-------|-------|----------|--------|---------------|-------|
| 001 | File Operations Hang | Critical | **FIXED** | v1.29.1 | 2024-12-20 |
| 002 | Function Definitions Cannot Be Executed | High | **Open** | v1.29.1 | 2025-08-21 |
| 003 | Array Indexing Not Implemented | Medium | **FIXED** | v1.29.1 | 2025-08-21 |
| 004 | v1.29.1 Release Compilation Failure | **CRITICAL** | **FIXED** | v1.29.1 | 2025-08-21 |
