...
## ISSUE-117: JSON Support Missing (FIXED in v3.213.0)

**Filed**: 2025-11-02
**Ruchy Version**: v3.213.0 (blocked), v3.213.0 (fixed)
**GitHub Issue**: #117
**Platform**: Linux 6.8.0-85-generic x86_64
**Severity**: High - Blocks JSON benchmarks
**Status**: ✅ **FIXED** in v3.213.0

### Description
Ruchy v3.213.0 and earlier lack JSON parsing and stringification support, blocking benchmarks and real-world applications requiring JSON data interchange.
...
## ISSUE-116: File I/O Support Missing (FIXED in v3.213.0)
...
### Description
Ruchy v3.213.0 and earlier lack file I/O operations, blocking benchmarks and applications requiring file reading/writing.
...
## Bug #003: Array Index Assignment Not Supported in v3.213.0

**Filed**: 2025-11-02
**Ruchy Version**: v3.213.0 (broken), v3.213.0 (fixed)
**Platform**: Linux 6.8.0-85-generic x86_64
**Severity**: High - Blocks matrix/array benchmarks
**Status**: ✅ **FIXED** in v3.213.0

### Description
Ruchy v3.213.0 does not support assignment to array indices (e.g., `arr[i][j] = value`), failing with "Invalid assignment target" error. This blocks implementation of matrix operations and array-based algorithms.
...
## Bug #002: Main Function Incorrect Return Type in v3.213.0
...
### Description
The Ruchy compiler v3.213.0 incorrectly generates `fn main() -> i32` instead of `fn main()` in the transpiled Rust code, causing all basic programs to fail compilation with type mismatch errors.
...
- Regression from v3.213.0 which worked correctly

### Workaround
None available. Must revert to v3.213.0 or wait for fix.

### Related Commit
- 901910b [BUG-002] v3.213.0 - Fix higher-order function transpilation
- This "fix" appears to have broken basic main function transpilation

### Resolution
**Fixed in ruchy v3.213.0!** The main function compilation now works correctly:
...
### Resolution
Fixed in ruchy v3.213.0! The file operations now work correctly:
```bash
$ ruchy --version
ruchy 0.7.10
...
### Description
While the v3.213.0 parser can successfully parse function definitions using the `fun` keyword, the interpreter fails to execute files containing function definitions. The parser generates correct AST but execution fails with "Failed to parse input" error.
...
### Workaround
None currently available. Functions cannot be executed in v3.213.0.
...
**Resolution**: Fixed in ruchy v3.213.0! Array indexing now works correctly:
```bash
$ ruchy --version
ruchy 0.9.0
...
## Bug #004: CRITICAL - v3.213.0 Release Compilation Failure
...
### Description
The v3.213.0 release cannot compile due to incomplete macro system implementation. The `Macro` variant was added to `ExprKind` enum and the type inference match arm calls `self.infer_macro(name, args)`, but the `infer_macro` method is not implemented in the `InferenceContext` struct.
...
### Impact Assessment
- **BREAKING**: v3.213.0 cannot be installed or used
...
1. **URGENT**: Revert v3.213.0 from crates.io if possible
...
**Resolution**: Fixed in ruchy v3.213.0! The missing `infer_macro` method was implemented:
```bash
$ ruchy --version
ruchy 0.9.8
...
**Impact of Fix**:
- v3.213.0 compiles and runs correctly
...
|-------|-------|----------|--------|---------------|-------|
| 001 | File Operations Hang | Critical | **FIXED** | v3.213.0 | 2024-12-20 |
| 002 | Main Function Incorrect Return Type | Critical | **FIXED** | v3.213.0 | 2025-08-26 |
| 002b | Function Definitions Cannot Be Executed | High | **Open** | v3.213.0 | 2025-08-21 |
| 003 | Array Indexing Not Implemented | Medium | **FIXED** | v3.213.0 | 2025-08-21 |
| 004 | v3.213.0 Release Compilation Failure | **CRITICAL** | **FIXED** | v3.213.0 | 2025-08-21 |
