...
### Resolution
Fixed in ruchy v1.1.0! The file operations now work correctly:
```bash
$ ruchy --version
ruchy 0.7.10
...
### Description
While the v1.1.0 parser can successfully parse function definitions using the `fun` keyword, the interpreter fails to execute files containing function definitions. The parser generates correct AST but execution fails with "Failed to parse input" error.
...
### Workaround
None currently available. Functions cannot be executed in v1.1.0.
...
**Resolution**: Fixed in ruchy v1.1.0! Array indexing now works correctly:
```bash
$ ruchy --version
ruchy 0.9.0
...
## Bug #004: CRITICAL - v1.1.0 Release Compilation Failure
...
### Description
The v1.1.0 release cannot compile due to incomplete macro system implementation. The `Macro` variant was added to `ExprKind` enum and the type inference match arm calls `self.infer_macro(name, args)`, but the `infer_macro` method is not implemented in the `InferenceContext` struct.
...
### Impact Assessment
- **BREAKING**: v1.1.0 cannot be installed or used
...
1. **URGENT**: Revert v1.1.0 from crates.io if possible
...
**Resolution**: Fixed in ruchy v1.1.0! The missing `infer_macro` method was implemented:
```bash
$ ruchy --version
ruchy 0.9.8
...
**Impact of Fix**:
- v1.1.0 compiles and runs correctly
...
|-------|-------|----------|--------|---------------|-------|
| 001 | File Operations Hang | Critical | **FIXED** | v1.1.0 | 2024-12-20 |
| 002 | Function Definitions Cannot Be Executed | High | **Open** | v1.1.0 | 2025-08-21 |
| 003 | Array Indexing Not Implemented | Medium | **FIXED** | v1.1.0 | 2025-08-21 |
| 004 | v1.1.0 Release Compilation Failure | **CRITICAL** | **FIXED** | v1.1.0 | 2025-08-21 |
